...
* [Orders](#deploymentorder) are generated from the [user's definition](#deployment).
* [Datacenters](#datacenters) [bid](#fulfillmentorder) on open [orders](#deploymentorder).
* The [bid](#fulfillmentorder) with lowest price gets [matched](#matchopenorders) with [order](#deploymentorder) to create a [lease](#lease).
...
* If a [datacenter](#datacenters) fails to maintain lease, collateral is transferred to user, and a new [order](#deploymentorder) is crated for the desired resources.
...
* Bidding on [deployment orders](#deploymentorder) fulfillable by the datacenter.
...
| wait-duration | Amount of time to wait before matching generated [deployment orders](#deploymentorder) with [fulfillment orders](#fulfillmentorder) |
...
[Deployment orders](#deploymentorder) are created from [deployment infrastructure](#deploymentinfrastructure) as necessary.
...
A `DeploymentOrder` is generated for each [deployment infrastructure](#deploymentinfrastructure) present in the [deployment](#deployment).
...
A `FulfillmentOrder` represents a [datacenter](#datacenters)'s interest in providing the resources requested in a [deployment order](#deploymentorder).

| Field | Definition |
| --- | --- |
| order | ID of [deployment order](#deploymentorder) which is being bid on. |
| resources | List of [resource groups](#resourcegroup) for this datacenter. |

The `resources` list must match the [deployment order's](#deploymentorder) `resources` list for each [resource group](#resourcegroup) with the following rules:

 * the `compute`, `count`,`collateral` fields must be the same.
 * the `price` field represents the [datacenter's](#datacenters) offering price and must be less than or equal to the [deployment order's](#deploymentorder) price.

The total collateral required to post a [fulfillment order](#fulfillmentorder) is the sum of `collateral` fields present in the [deployment order's](#deploymentorder) `resources` list.

#### Lease

A `Lease` represents a matching [deployment order](#deploymentorder) and [fulfillment order](#fulfillmentorder).

| Field | Definition |
| --- | --- |
| deployment-order | ID of [deployment order](#deploymentorder) |
| fulfillment-order | ID of [fulfillment order](#deploymentorder) |
...
Sent by a [user](#users) to deploy their application on Photon.  A [deployment order](#deploymentorder) will be created for each datacenter
...
Sent by a [datacenter](#datacenters) to bid on a [deployment order](#deploymentorder).
...
Sent by a [validator](#validator) to match a [deployment order](#deploymentorder) with a [fulfillment order](#fulfillmentorder).
...
For each [deployment order](#deploymentorder) that is ready to be fulfilled (`state=open`,`wait-duration` has transpired):
...
For each open [deployment order](#deploymentorder):

1. If the [datacenter](#datacenters) is out of collateral, exit.
1. If [datacenter](#datacenters) is not able to fulfill the [deployment order](#deploymentorder), skip to next [deployment order](#deploymentorder).
1. Emit a [`SubmitFulfillmentOrder`](#submitfulfillmentorder) transaction for the [deployment order](#deploymentorder)

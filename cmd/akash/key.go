...
func keyCommand() *cobra.Command {
	...
	cmd.AddCommand(keyDeleteCommand())
	return cmd
}

func keyDeleteCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:     "delete <name>",
		...
		RunE:    session.WithSession(session.RequireRootDir(doKeyDeleteCommand)),
	}
	session.AddFlagKeyType(cmd, cmd.Flags())
	return cmd
}

func doKeyDeleteCommand(ses session.Session, cmd *cobra.Command, args []string) error {
	var name string
	if len(args) == 1 {
		name = args[0]
	}

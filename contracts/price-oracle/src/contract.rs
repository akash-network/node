use cosmwasm_std::{
    ...
    QuerierWrapper, Response, StdResult, Uint128, Uint256,
};
...
use crate::oracle::{pyth_price_to_decimal, MsgAddDenomPriceEntry};
...
pub fn execute_update_price_feed(
    ...
    // Pyth uses i64 for price, we need to convert Uint128 to i64
    let price_i64 = price.u128() as i64;
    let price_decimal = pyth_price_to_decimal(price_i64, expo);

    // Create oracle message data (will be handled by a relayer or chain module)
    let oracle_msg = MsgAddDenomPriceEntry::new(
        env.contract.address.to_string(), // Contract address as signer
        "akt".to_string(),                 // Denomination
        price_decimal.clone(),             // Decimal price string (cloned)
    );

    // Encode to protobuf for potential external relayer
    ...
    Ok(Response::new()
        ...
        .add_attribute("oracle_price", price_decimal) // Decimal price for oracle
        .add_attribute("oracle_denom", "akt") // Denomination
        .add_attribute("oracle_data", oracle_data.to_base64()) // Encoded protobuf data

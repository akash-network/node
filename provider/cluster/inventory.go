...
func (is *inventoryService) updateInventoryMetrics(metrics ctypes.InventoryMetrics) {
	...
	clusterInventoryAllocateable.WithLabelValues("cpu").Set(metrics.TotalAllocatable.CPU)
	...
	clusterInventoryAvailable.WithLabelValues("cpu").Set(metrics.TotalAvailable.CPU)

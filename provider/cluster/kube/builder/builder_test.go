...
func TestLidNsSanity(t *testing.T) {
	...
	mb := NewManifest(log, settings, leaseID, g)
...
// 	g := &manitypes.Group{}
// 	settings := NewDefaultSettings()
// 	np := NewNetPol(NewDefaultSettings(), leaseID, g)
//
// 	// disabled
...
// 	np = NewNetPol(settings, leaseID, g)
// 	netPolicies, err = np.Create()
// 	assert.NoError(t, err)
...
func TestGlobalServiceBuilder(t *testing.T) {
	...
	serviceBuilder := NewService(myLog, mySettings, lid, group, service, true)
...
func TestLocalServiceBuilder(t *testing.T) {
	...
	mySettings := NewDefaultSettings()
	lid := testutil.LeaseID(t)
	serviceBuilder := NewService(myLog, mySettings, lid, group, service, false)
	require.NotNil(t, serviceBuilder)

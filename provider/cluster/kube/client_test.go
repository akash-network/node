...
func TestShouldExpose(t *testing.T){
	...
	clientInterface, err := NewClient(myLog, "thehost", "thenamespace")
	...
	require.False(t, client.shouldExpose(&manifest.ServiceExpose{
		Global: true,
		Proto: manifest.TCP,
		Port: 81,
	}))
	...
	require.True(t, client.shouldExpose(&manifest.ServiceExpose{
		Global: true,
		Proto: manifest.TCP,
		Port: 80,
	}))
...
func TestLeaseStatusWithForwardedPortOnly(t *testing.T){
	...
	servicesList.Items[0].Name = "myservice" + suffixForGlobalServiceName

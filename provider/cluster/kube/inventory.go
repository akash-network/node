...
func (c *client) fetchStorage(ctx context.Context) (clusterStorage, error) {
	...
	svcResult, err := c.kc.CoreV1().Services("").List(ctx, metav1.ListOptions{
		LabelSelector: builder.AkashManagedLabelName + "=true,app.kubernetes.io/name=akash,app.kubernetes.io/instance=inventory,app.kubernetes.io/component=operator",
	})
	if err != nil {
		return nil, err
	}

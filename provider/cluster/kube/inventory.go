...
func (c *client) Inventory(ctx context.Context) (ctypes.Inventory, error) {
	cstorage, err := c.fetchClusterStorage(ctx)
	if err != nil {
		return nil, err
	}
...
func (c *client) fetchClusterStorage(ctx context.Context) (clusterStorage, error) {
	...
	storageInfo, err := c.ac.AkashV1().StorageClassInfos().List(ctx, akashManagedOpts)
	if err != nil && !apierrors.IsNotFound(err) {
		return nil, err
	}

	for _, state := range storageInfo.Items {
		// storage class may not yet be defined
		if sclass, exists := storage[state.Name]; exists {
			sclass.allocatable = *resource.NewQuantity(int64(state.Spec.Capacity), resource.DecimalSI)
		}
	}

...
#	version=$($result version -o json | jq -rj '.minikubeVersion')
#	"$CURDIR"/semver.sh compare "$version" "$MINIKUBE_MIN_VERSION"
#
#	# shellcheck disable=SC2181
#	if [ ! $? -ge 0 ]; then
#		echo "detect unsupported version of minikube $version. min required $MINIKUBE_MIN_VERSION"
#		exit 1
#	fi
...
ip)
	case $VM_DRIVER in
	parallels)
		prlctl list -f -j | jq -jr '.[] | select(.name == "minikube") | .ip_configured'
		;;
	esac
	;;
...
akash-setup)
	...
	kubectl_retry apply -f "${AKASH_ROOT}/_docs/rook/storageclassinfo.yaml"

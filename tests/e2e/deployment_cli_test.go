...
func (s *deploymentIntegrationTestSuite) TestFundedDeployment() {
	...
	prevFunderActBal := s.getAccountBalanceOf(s.addrFunder, DefaultDeposit.Denom)

	ctx := context.Background()

	// Creating deployment paid by the funder's account without any authorization from the funder should fail
	_, err = clitestutil.ExecDeploymentCreate(
		ctx,
		s.cctx,
		cli.TestFlags().
	...
	s.Require().Equal(prevFunderActBal, s.getAccountBalanceOf(s.addrFunder, DefaultDeposit.Denom))
	...
	ownerActBal := s.getAccountBalanceOf(s.addrDeployer, DefaultDeposit.Denom)
	...
	s.Require().NoError(err)
	s.Require().NoError(s.Network().WaitForNextBlock())
	_, protoTx := clitestutil.ValidateTxSuccessful(ctx, s.T(), s.cctx, res.Bytes())

	// funder's uact balance should be deducted correctly
	curFunderActBal := s.getAccountBalanceOf(s.addrFunder, DefaultDeposit.Denom)
	s.Require().Equal(prevFunderActBal.Sub(DefaultDeposit.Amount), curFunderActBal)
	prevFunderActBal = curFunderActBal

	// owner's balance should be deducted for fees correctly
	curOwnerBal := s.getAccountBalance(s.addrDeployer)
	s.Require().Equal(ownerBal.SubRaw(protoTx.AuthInfo.Fee.Amount.AmountOf("uakt").Int64()), curOwnerBal)
	...
	res, err = clitestutil.ExecEscrowDeposit(
		...
		s.cctx,
		cli.TestFlags().
			With("deployment", DefaultDeposit.String()).
	...
	// owner's uakt balance should be deducted for fees only
	curOwnerBal = s.getAccountBalance(s.addrDeployer)
	s.Require().Equal(ownerBal.SubRaw(protoTx.AuthInfo.Fee.Amount.AmountOf("uakt").Int64()), curOwnerBal)
	ownerBal = curOwnerBal
	// owner's uact balance should be deducted by deposit amount
	curOwnerActBal := s.getAccountBalanceOf(s.addrDeployer, DefaultDeposit.Denom)
	s.Require().Equal(ownerActBal.Sub(DefaultDeposit.Amount), curOwnerActBal)
	ownerActBal = curOwnerActBal
	...
	// funder gets back 2x deposit in uact (creation + additional grant deposit)
	s.Require().Equal(prevFunderActBal.Add(DefaultDeposit.Amount.MulRaw(2)), s.getAccountBalanceOf(s.addrFunder, DefaultDeposit.Denom))
	// deployer gets back 1x deposit in uact (balance deposit), and pays close fees in uakt
	s.Require().Equal(ownerActBal.Add(DefaultDeposit.Amount), s.getAccountBalanceOf(s.addrDeployer, DefaultDeposit.Denom))
	s.Require().Equal(ownerBal.SubRaw(protoTx.AuthInfo.Fee.Amount.AmountOf("uakt").Int64()), s.getAccountBalance(s.addrDeployer))
}
...
func (s *deploymentIntegrationTestSuite) getAccountBalanceOf(address sdk.AccAddress, denom string) sdkmath.Int {
	ctx := context.Background()
	cctxJSON := s.Network().Validators[0].ClientCtx.WithOutputFormat("json")
	res, err := clitestutil.QueryBalancesExec(ctx, cctxJSON, address.String())
	s.Require().NoError(err)
	...
	err = cctxJSON.Codec.UnmarshalJSON(res.Bytes(), &balRes)
	s.Require().NoError(err)
	return balRes.Balances.AmountOf(denom)
}

...
func (up *upgrade) migrateDenoms(ctx context.Context) error {
	...
	ekeeper.WithPayments(sctx, func(p etypes.Payment) bool {
		...
		// Track Payment.Balance for burn/mint (1:1 ratio for axlUSDC)
		balAmt := p.State.Balance.Amount.TruncateInt()
		if balAmt.GT(sdkmath.ZeroInt()) {
			origDenom := p.State.Balance.Denom
			if burnCoin.IsNil() {
				burnCoin = sdk.NewCoin(origDenom, sdkmath.ZeroInt())
			}
			burnCoin = burnCoin.Add(sdk.NewCoin(origDenom, balAmt))
			mintCoin = mintCoin.Add(sdk.NewCoin(sdkutil.DenomUact, balAmt))
		}

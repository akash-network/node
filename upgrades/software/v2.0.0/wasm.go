...
func (up *upgrade) instantiateOracleContracts(ctx context.Context) (string, error) {
	...
	switch {
	case sctx.ChainID() == "akashnet-2": // mainnet is multisig 3/5
		adminAddr = ""
	case strings.HasPrefix(sctx.ChainID(), "sandbox"): // sandbox networks
		fallthrough
	case strings.HasPrefix(sctx.ChainID(), "testnet"): // sandbox networks
		adminAddr = "akash15alm48ghrjwz59xw4wmm85uurz82y2dt69gtlg"
	case strings.HasPrefix(sctx.ChainID(), "localakash"): // local network used for testing network upgrades
		adminAddr = "akash1kwngmq9s7mfs6x7aes5t0jcm5yvmd7367jm3ke"
	default:
		panic(fmt.Errorf("unsupported chain ID: %s", sctx.ChainID()))
	}

	// Store and instantiate Wormhole contract
	wormholeAddr, err := up.storeAndInstantiateWormhole(ctx, msgServer, govAddr, adminAddr)
	if err != nil {
	...
	pythAddr, err := up.storeAndInstantiatePyth(ctx, msgServer, govAddr, adminAddr, wormholeAddr)
	if err != nil {
...
func (up *upgrade) storeAndInstantiateWormhole(
	...
	codeID, err := storeContract(ctx, msgServer, govAddr, path.Join(node.ContractsDir, "wormhole.wasm"))
	if err != nil {
...
func (up *upgrade) storeAndInstantiatePyth(
	...
	codeID, err := storeContract(ctx, msgServer, govAddr, path.Join(node.ContractsDir, "pyth.wasm"))
	if err != nil {
		return "", fmt.Errorf("store code: %w", err)
	}
...
func storeContract(
	...
	wasmBz, err := node.Contracts.ReadFile(path)
	if err != nil {
		return 0, fmt.Errorf("read embedded wasm %s: %w", path, err)
	}

...
func NewQuerier(keeper keeper.Keeper, legacyQuerierCdc *codec.LegacyAmino) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) ([]byte, error) {
		...
		case tokenAuditor:
			return auditorList(ctx, path[3:], req, keeper, legacyQuerierCdc)
		}
...
func auditorList(ctx sdk.Context, path []string, _ abci.RequestQuery, keeper keeper.Keeper, legacyQuerierCdc *codec.LegacyAmino) ([]byte, error) {
	var values types.Providers
	...
	auditor, err := sdk.AccAddressFromBech32(path[0])
	if err != nil {
		return nil, types.ErrInvalidAddress
	}
	...
	if path[1] == tokenList {
		...
		res, exists := keeper.GetProviderByAuditor(ctx, types.ProviderID{
			Owner:   owner,
			Auditor: auditor,
		})

...
func (k *keeper) prepareToBM(sctx sdk.Context, burnCoin sdk.Coin, toDenom string) (bmetypes.CoinPrice, bmetypes.CoinPrice, error) {
	...
	// check if the result in base units is less than the denom's precision
	// e.g. for uakt (10^-6): mintAmountDec * 10^-6 must be >= 10^-6
	if mintAmountDec.Mul(denomUnit).LT(denomUnit) {
		return bmetypes.CoinPrice{}, bmetypes.CoinPrice{}, bmetypes.ErrEpsilon.Wrapf("result %s is less than precision of %s (%s)", mintAmountDec, toDenom, denomUnit)
	}

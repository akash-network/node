...
type Keeper interface {
	...
	IterateLedgerFailedRecords(sdk.Context, func(bmetypes.LedgerRecordID, bmetypes.LedgerFailedRecord) (bool, error)) error
...
type keeper struct {
	...
	ledgerFailed          collections.Map[bmetypes.LedgerRecordID, bmetypes.LedgerFailedRecord]
...
func NewKeeper(
	...
	k := &keeper{
		...
		ledgerFailed:          collections.NewMap(sb, LedgerFailedKey, "ledger_failed", ledgerRecordIDCodec{}, codec.CollValue[bmetypes.LedgerFailedRecord](cdc)),
...
func (k *keeper) IterateLedgerFailedRecords(sctx sdk.Context, f func(bmetypes.LedgerRecordID, bmetypes.LedgerFailedRecord) (bool, error)) error {
	return k.ledgerFailed.Walk(sctx, nil, f)
}
...
func (k *keeper) executeBurnMint(
	...
	bmErr := func() error {
		...
		if burn.Coin.Denom == sdkutil.DenomUakt {
			return k.mintACT(sctx, id, burn, mint, srcAddr, dstAddr, postRun)
		}

		return k.burnACT(sctx, id, burn, mint, srcAddr, dstAddr, postRun)
	}()

	if bmErr != nil {
		return k.failBurnMint(sctx, id, srcAddr, dstAddr, burnCoin, toDenom, bmErr)
	}

	return nil
}

// failBurnMint records a failed burn/mint operation, refunds coins to the owner,
// and cleans up the pending ledger state.
func (k *keeper) failBurnMint(
	sctx sdk.Context,
	id bmetypes.LedgerRecordID,
	srcAddr sdk.AccAddress,
	dstAddr sdk.AccAddress,
	...
	failReason := bmetypes.BMFailReasonUnknown
	if errors.Is(reason, bmetypes.ErrEpsilon) {
		failReason = bmetypes.BMFailReasonEpsilon
	}

	if err := k.ledgerFailed.Set(sctx, id, bmetypes.LedgerFailedRecord{
	...
	err = sctx.EventManager().EmitTypedEvent(&bmetypes.EventLedgerRecordFailed{
		ID:          id,
		FailReason:  failReason,
		Owner:       srcAddr.String(),
		To:          dstAddr.String(),
...
func (k *keeper) prepareToBM(sctx sdk.Context, burnCoin sdk.Coin, toDenom string) (bmetypes.CoinPrice, bmetypes.CoinPrice, error) {
	...
	if priceFrom.IsZero() || priceTo.IsZero() {
		return bmetypes.CoinPrice{}, bmetypes.CoinPrice{}, bmetypes.ErrZeroPrice.Wrapf("oracle prices must be non-zero (%s=%s, %s=%s)", burnCoin.Denom, priceFrom, toDenom, priceTo)
	}
	...
	if !found {
		return bmetypes.CoinPrice{}, bmetypes.CoinPrice{}, bmetypes.ErrInvalidDenom.Wrapf("denom %s is not registered", toDenom)
	}
	...
	if !mintAmount.IsPositive() {
		return bmetypes.CoinPrice{}, bmetypes.CoinPrice{}, bmetypes.ErrEpsilon.Wrapf("result %s truncates to zero for %s (denomUnit %s)", mintAmountDec, toDenom, denomUnit)
	}

...
func setupTestSuite(t *testing.T) *testSuite {
	...
	authzKeeper.
		...
		Run(func(args mock.Arguments) {
			...
			authorization := &ev1.DepositAuthorization{
				...
				SpendLimits: sdk.Coins{defaultDeposit.Add(defaultDeposit)},
			}
		...
		Run(func(args mock.Arguments) {
			...
			authorization := &ev1.DepositAuthorization{
				Scopes:      ev1.DepositAuthorizationScopes{ev1.DepositScopeDeployment},
				SpendLimits: sdk.Coins{defaultDeposit},
			}
	...
	suite.dhandler = handler.NewHandler(suite.dkeeper, suite.mkeeper, ssuite.EscrowKeeper(), ssuite.BmeKeeper(), ssuite.BankKeeper())

...
type grpcTestSuite struct {
	*state.TestSuite
	...
	bankKeeper  eimports.BankKeeper
...
func TestGRPCQueryDeployments(t *testing.T) {
	...
	t.Run("offset without state filter returns error", func(t *testing.T) {
		_, err := suite.queryClient.Deployments(suite.ctx, &dvbeta.QueryDeploymentsRequest{
			Pagination: &sdkquery.PageRequest{Offset: 1, Limit: 1},
		})
		require.Error(t, err)
	})
	...
	t.Run("NextKey set when more results exist", func(t *testing.T) {
		res, err := suite.queryClient.Deployments(suite.ctx, &dvbeta.QueryDeploymentsRequest{
			Pagination: &sdkquery.PageRequest{Limit: 1},
		})
		require.NoError(t, err)
	...
	t.Run("key-based pagination returns next page", func(t *testing.T) {
		...
		page2, err := suite.queryClient.Deployments(suite.ctx, &dvbeta.QueryDeploymentsRequest{
		...
		page3, err := suite.queryClient.Deployments(suite.ctx, &dvbeta.QueryDeploymentsRequest{

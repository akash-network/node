...
func TestGRPCQueryDeploymentsWithFilter(t *testing.T) {
	...
	for _, depID := range deps {
		for _, m := range modifiers {
			...
			// At least 1 result
			require.GreaterOrEqual(t, len(res.Deployments), 1, "testing %v", m.fieldName)

			for _, dep := range res.Deployments {
				require.Equal(t, m.getField(depID), m.getField(dep.Deployment.DeploymentID), "testing %v", m.fieldName)
			}
		}
	}

...
func (m Migration) migrateEscrow(
	...
	for _, pmnt := range payments {
		...
		// Track pre-conversion Payment.Balance for burn/mint
		if pmnt.State.Balance.Denom == fromDenom && pmnt.State.Balance.Amount.GTE(sdkmath.LegacyZeroDec()) {
			srcCoin = srcCoin.Add(sdk.NewDecCoinFromDec(fromDenom, pmnt.State.Balance.Amount))
		}
		...
		if pmnt.State.Balance.Denom == fromDenom {
			...
			changed = true
			// Track post-conversion Payment.Balance for mint
			if pmnt.State.Balance.Amount.GTE(sdkmath.LegacyZeroDec()) {
				dstCoin = dstCoin.Add(sdk.NewDecCoinFromDec(toDenom, pmnt.State.Balance.Amount))
			}
		}

...
// GetEpoch returns epoch info by identifier.
func (k *keeper) GetEpoch(ctx sdk.Context, identifier string) (types.EpochInfo, error) {
	return k.EpochInfo.Get(ctx, identifier)
}

// AddEpoch adds a new epoch info. Will return an error if the epoch fails validation,
// or re-uses an existing identifier.
// This method also sets the start time if left unset, and sets the epoch start height.
func (k *keeper) AddEpoch(ctx sdk.Context, epoch types.EpochInfo) error {
	err := epoch.Validate()
	if err != nil {
		return err
	}
	// Check if identifier already exists
	isExist, err := k.EpochInfo.Has(ctx, epoch.ID)
	if err != nil {
		return err
	}
	if isExist {
		return fmt.Errorf("epoch with identifier %s already exists", epoch.ID)
	}
	...
	return k.EpochInfo.Set(ctx, epoch.ID, epoch)
}

func (k *keeper) RemoveEpoch(sctx sdk.Context, id string) error {
	_, err := k.EpochInfo.Get(sctx, id)
	if err != nil {
		return fmt.Errorf("epoch with identifier %s not found", id)
	}

	return k.EpochInfo.Remove(sctx, id)
}

// IterateEpochs iterate through epochs to return all epochs info.
func (k *keeper) IterateEpochs(ctx sdk.Context, fn func(string, types.EpochInfo) (bool, error)) error {
	return k.EpochInfo.Walk(ctx, nil, fn)
}

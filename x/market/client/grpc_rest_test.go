...
func (s *IntegrationTestSuite) TestGetOrders() {
	...
	testCases := []struct {
		...
		},
		{
			...
			fmt.Sprintf("%s/akash/market/v1beta1/orders/list?filters.state=%d&filters.oseq=%d",
				val.APIAddress, order.State, order.OrderID.OSeq),
			false,
			order,
			1,
		},
...
func (s *IntegrationTestSuite) TestGetBids() {
	...
	testCases := []struct {
		...
		},
		{
			...
			fmt.Sprintf("%s/akash/market/v1beta1/bids/list?filters.state=%d&filters.oseq=%d&filters.provider=%s",
				val.APIAddress, bid.State, bid.BidID.OSeq, providerAddrBase64),
			false,
			bid,
			1,
		},
...
func (s *IntegrationTestSuite) TestGetLeases() {
	...
	testCases := []struct {
		...
		},
		{
			...
			fmt.Sprintf("%s/akash/market/v1beta1/leases/list?filters.state=%d&filters.oseq=%d&filters.provider=%s",
				val.APIAddress, lease.State, lease.LeaseID.OSeq, providerAddrBase64),
			false,
			lease,
			1,
		},

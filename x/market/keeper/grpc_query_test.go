...
type grpcTestSuite struct {
	*state.TestSuite
	...
	queryClient mvbeta.QueryClient
}
...
func setupTest(t *testing.T) *grpcTestSuite {
	...
	mvbeta.RegisterQueryServer(queryHelper, querier)
	suite.queryClient = mvbeta.NewQueryClient(queryHelper)
...
func TestGRPCQueryOrder(t *testing.T) {
	...
	var (
		req      *mvbeta.QueryOrderRequest
		expOrder mvbeta.Order
	)
	...
	testCases := []struct {
		...
		},
		{
			"invalid request",
			func() {
				req = &mvbeta.QueryOrderRequest{ID: mv1.OrderID{}}
			},
			false,
		},
		{
		...
		},
		{
			"success",
			func() {
				req = &mvbeta.QueryOrderRequest{ID: order.ID}
				expOrder = order
			},
			true,
		},
...
func TestGRPCQueryOrders(t *testing.T) {
	...
	var req *mvbeta.QueryOrdersRequest
	...
	testCases := []struct {
			...
			func() {
				req = &mvbeta.QueryOrdersRequest{
					Filters: mvbeta.OrderFilters{
						OSeq:  37,
						State: mvbeta.OrderActive.String(),
					}}
			},
			0,
		},
		{
			...
			func() {
				req = &mvbeta.QueryOrdersRequest{Filters: mvbeta.OrderFilters{State: mvbeta.OrderActive.String()}}
			},
			1,
		},
		{
			...
			func() {
				req = &mvbeta.QueryOrdersRequest{Pagination: &sdkquery.PageRequest{Limit: 1}}
			},
			1,
		},
	}
...
type orderFilterModifier struct {
	fieldName string
	f         func(orderID mv1.OrderID, filter mvbeta.OrderFilters) mvbeta.OrderFilters
	getField  func(orderID mv1.OrderID) interface{}
}

type bidFilterModifier struct {
	fieldName string
	f         func(bidID mv1.BidID, filter mvbeta.BidFilters) mvbeta.BidFilters
...
func TestGRPCQueryOrdersWithFilter(t *testing.T) {
	...
	modifiers := []orderFilterModifier{
		{
			"owner",
			func(orderID mv1.OrderID, filter mvbeta.OrderFilters) mvbeta.OrderFilters {
	...
	for _, orderID := range orders {
		for _, m := range modifiers {
			req := &mvbeta.QueryOrdersRequest{
				Filters: m.f(orderID, mvbeta.OrderFilters{}),
			}
...
func TestGRPCQueryBidsWithFilter(t *testing.T) {
	...
	modifiers := []bidFilterModifier{
		{
			"owner",
			func(bidID mv1.BidID, filter mvbeta.BidFilters) mvbeta.BidFilters {
...
func TestGRPCQueryBid(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			"invalid request",
			func() {
				req = &mvbeta.QueryBidRequest{ID: mv1.BidID{}}
			},
			false,
		},
		{
		...
		},
		{
			"success",
			func() {
				req = &mvbeta.QueryBidRequest{ID: bid.ID}
				expBid = bid
			},
			true,
		},
...
func TestGRPCQueryBids(t *testing.T) {
	...
	var req *mvbeta.QueryBidsRequest
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				req = &mvbeta.QueryBidsRequest{Filters: mvbeta.BidFilters{State: mvbeta.BidLost.String()}}
			},
			1,
		},
		{
			...
			func() {
				req = &mvbeta.QueryBidsRequest{Pagination: &sdkquery.PageRequest{Limit: 1}}
			},
			1,
		},
	}
...
func TestGRPCQueryLease(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			"invalid request",
			func() {
				req = &mvbeta.QueryLeaseRequest{ID: mv1.LeaseID{}}
			},
			false,
		},
		{
		...
		},
		{
			"success",
			func() {
				req = &mvbeta.QueryLeaseRequest{ID: lease.ID}
				expLease = lease
			},
			true,
		},
...
func TestGRPCQueryLeases(t *testing.T) {
	...
	var req *mvbeta.QueryLeasesRequest
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				req = &mvbeta.QueryLeasesRequest{Pagination: &sdkquery.PageRequest{Limit: 1}}
			},
			1,
		},
	}

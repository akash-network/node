...
func SimulateMsgCreateBid(ks keepers.Keepers) simtypes.Operation {
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accounts []simtypes.Account, chainID string) (simtypes.OperationMsg, []simtypes.FutureOperation, error) {
		orders := getOrdersWithState(ctx, ks, mtypes.OrderOpen)
		if len(orders) == 0 {
			return simtypes.NoOpMsg(mtypes.ModuleName, (&mtypes.MsgCreateBid{}).Type(), "no open orders found"), nil, nil
		}
		...
		if len(providers) == 0 {
			return simtypes.NoOpMsg(mtypes.ModuleName, (&mtypes.MsgCreateBid{}).Type(), "no providers found"), nil, nil
		}
		...
		if convertErr != nil {
			return simtypes.NoOpMsg(mtypes.ModuleName, (&mtypes.MsgCreateBid{}).Type(), "error while converting address"), nil, convertErr
		}

		simAccount, found := simtypes.FindAccount(accounts, ownerAddr)
		if !found {
			return simtypes.NoOpMsg(mtypes.ModuleName, (&mtypes.MsgCreateBid{}).Type(), "unable to find provider"),
				nil, fmt.Errorf("provider with %s not found", provider.Owner)
		}

		if provider.Owner == order.ID.Owner {
			return simtypes.NoOpMsg(mtypes.ModuleName, (&mtypes.MsgCreateBid{}).Type(), "provider and order owner cannot be same"),
				nil, nil
		}
		...
		if spendable.AmountOf(depositAmount.Denom).LT(depositAmount.Amount.MulRaw(2)) {
			return simtypes.NoOpMsg(mtypes.ModuleName, (&mtypes.MsgCreateBid{}).Type(), "out of money"), nil, nil
		}
		spendable = spendable.Sub(depositAmount)

		fees, err := simtypes.RandomFees(r, ctx, spendable)
		if err != nil {
			return simtypes.NoOpMsg(mtypes.ModuleName, (&mtypes.MsgCreateBid{}).Type(), "unable to generate fees"), nil, err
		}

		msg := mtypes.NewMsgCreateBid(mtypes.MakeBidID(order.ID, simAccount.Address), order.Prices(), deposit.Deposit{
		...
		if err != nil {
			return simtypes.NoOpMsg(mtypes.ModuleName, msg.Type(), "unable to generate mock tx"), nil, err
		}
		...
		case errors.Is(err, mtypes.ErrBidExists):
			return simtypes.NewOperationMsg(msg, false, ""), nil, nil
		default:
			return simtypes.NoOpMsg(mtypes.ModuleName, msg.Type(), "unable to deliver mock tx"), nil, err
		}
	}
}
...
func SimulateMsgCloseBid(ks keepers.Keepers) simtypes.Operation {
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accounts []simtypes.Account,
		...
		ks.Market.WithBids(ctx, func(bid mtypes.Bid) bool {
			if bid.State == mtypes.BidActive {
				lease, ok := ks.Market.GetLease(ctx, mtypes.LeaseID(bid.ID))
				if ok && lease.State == mtypes.LeaseActive {
		...
		if len(bids) == 0 {
			return simtypes.NoOpMsg(mtypes.ModuleName, (&mtypes.MsgCloseBid{}).Type(), "no matched bids found"), nil, nil
		}
		...
		if convertErr != nil {
			return simtypes.NoOpMsg(mtypes.ModuleName, (&mtypes.MsgCloseBid{}).Type(), "error while converting address"), nil, convertErr
		}

		simAccount, found := simtypes.FindAccount(accounts, providerAddr)
		if !found {
			return simtypes.NoOpMsg(mtypes.ModuleName, (&mtypes.MsgCloseBid{}).Type(), "unable to find bid with provider"),
		...
		fees, err := simtypes.RandomFees(r, ctx, spendable)
		if err != nil {
			return simtypes.NoOpMsg(mtypes.ModuleName, (&mtypes.MsgCloseBid{}).Type(), "unable to generate fees"), nil, err
		}

		msg := mtypes.NewMsgCloseBid(bid.ID, mtypes.LeaseClosedReasonUnspecified)
		...
		_, _, err = app.SimDeliver(txGen.TxEncoder(), tx)
		if err != nil {
			return simtypes.NoOpMsg(mtypes.ModuleName, msg.Type(), "unable to deliver tx"), nil, err
		}
...
func SimulateMsgCloseLease(_ keepers.Keepers) simtypes.Operation {
	return func(
		...
		return simtypes.NoOpMsg(mtypes.ModuleName, (&mtypes.MsgCloseLease{}).Type(), "skipping"), nil, nil
	}
}

...
func validateCoin(i interface{}) error {
	_, ok := i.(sdk.Coin)
	if !ok {
		return errors.Wrapf(types.ErrInvalidParam, "invalid type %T", i)
	}
...
func validateOrderMaxBids(i interface{}) error {
	...
	if val == 0 {
		return errors.Wrap(types.ErrInvalidParam, "order max bids too low")
	}

	if val > maxOrderMaxBids {
		return errors.Wrap(types.ErrInvalidParam, "order max bids too high")
	}

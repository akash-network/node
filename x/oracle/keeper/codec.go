...
// priceDataRecordIDLegacyCodec implements codec.KeyCodec[PriceDataID]
type priceDataRecordIDCodec struct{}

// priceDataRecordIDLegacyCodec implements codec.KeyCodec[PriceDataID]
type priceDataRecordIDLegacyCodec struct{}
...
func (d priceDataRecordIDCodec) Encode(buffer []byte, key types.PriceDataRecordID) (int, error) {
	offset := 0
	// Write source id as big-endian uint32 (for proper ordering)
	binary.BigEndian.PutUint32(buffer, key.Source)
	offset += 4
	...
	offset += copy(buffer[offset:], EncodeTimestamp(key.Timestamp))

	binary.BigEndian.PutUint64(buffer[offset:], key.Sequence)
	offset += 8
...
func (d priceDataRecordIDCodec) Decode(buffer []byte) (int, types.PriceDataRecordID, error) {
	...
	err = validation.KeyAtLeastLength(buffer, TimestampEncodedSize)
	if err != nil {
		return 0, types.PriceDataRecordID{}, err
	}

	res.Timestamp, err = DecodeTimestamp(buffer[:TimestampEncodedSize])
	if err != nil {
		return 0, types.PriceDataRecordID{}, err
	}

	buffer = buffer[TimestampEncodedSize:]
	decodedLen += TimestampEncodedSize
	...
	res.Sequence = binary.BigEndian.Uint64(buffer)
	decodedLen += 8
...
func (d priceDataRecordIDCodec) Size(key types.PriceDataRecordID) int {
	...
	return 4 + ln + TimestampEncodedSize + 8
}
...
func (d priceDataRecordIDCodec) Stringify(key types.PriceDataRecordID) string {
	return fmt.Sprintf("%d/%s/%s/%s/%d", key.Source, key.Denom, key.BaseDenom, key.Timestamp.UTC().Format(time.RFC3339Nano), key.Sequence)
}
...
func (d priceDataRecordIDLegacyCodec) Encode(buffer []byte, key types.PriceDataRecordID) (int, error) {
	offset := 0
	// Write source id as big-endian uint64 (for proper ordering)
	binary.BigEndian.PutUint32(buffer, key.Source)
	offset += 4
	...
	binary.BigEndian.PutUint64(buffer[offset:], uint64(key.Timestamp.UTC().UnixNano()))
	offset += 8

	return offset, nil
}

func (d priceDataRecordIDLegacyCodec) Decode(buffer []byte) (int, types.PriceDataRecordID, error) {
	err := validation.KeyAtLeastLength(buffer, 5)
	if err != nil {
		return 0, types.PriceDataRecordID{}, err
	}
	...
	res.Timestamp = time.Unix(0, int64(binary.BigEndian.Uint64(buffer))).UTC()
...
func (d priceDataRecordIDLegacyCodec) Size(key types.PriceDataRecordID) int {
	ln := len(conv.UnsafeStrToBytes(key.Denom)) + 1
	ln += len(conv.UnsafeStrToBytes(key.BaseDenom)) + 1
...
func (d priceDataRecordIDLegacyCodec) EncodeJSON(key types.PriceDataRecordID) ([]byte, error) {
	return json.Marshal(key)
}

func (d priceDataRecordIDLegacyCodec) DecodeJSON(b []byte) (types.PriceDataRecordID, error) {
	var key types.PriceDataRecordID
	err := json.Unmarshal(b, &key)
	return key, err
}

func (d priceDataRecordIDLegacyCodec) Stringify(key types.PriceDataRecordID) string {
	return fmt.Sprintf("%d/%s/%s/%s", key.Source, key.Denom, key.BaseDenom, key.Timestamp.UTC().Format(time.RFC3339Nano))
}

func (d priceDataRecordIDLegacyCodec) KeyType() string {
	return "PriceDataRecordID"
}
...
func (d priceDataRecordIDLegacyCodec) EncodeNonTerminal(buffer []byte, key types.PriceDataRecordID) (int, error) {
	return d.Encode(buffer, key)
}

func (d priceDataRecordIDLegacyCodec) DecodeNonTerminal(buffer []byte) (int, types.PriceDataRecordID, error) {
	return d.Decode(buffer)
}

func (d priceDataRecordIDLegacyCodec) SizeNonTerminal(key types.PriceDataRecordID) int {
	return d.Size(key)
}

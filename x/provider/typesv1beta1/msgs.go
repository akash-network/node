...
func NewMsgCreateProvider(owner sdk.AccAddress, hostURI string, attributes typesv1beta1.Attributes) *MsgCreateProvider {
...
func NewMsgUpdateProvider(owner sdk.AccAddress, hostURI string, attributes typesv1beta1.Attributes) *MsgUpdateProvider {
...
func validateProviderURI(val string) error {
	u, err := url.Parse(val)
	if err != nil {
		return types.ErrInvalidProviderURI
	}
	if !u.IsAbs() {
		return errors.Wrapf(types.ErrNotAbsProviderURI, "validating %q for absolute URI", val)
	}

	if u.Scheme != "https" {
		return errors.Wrapf(types.ErrInvalidProviderURI, "scheme in %q should be https", val)
	}

	if u.Host == "" {
		return errors.Wrapf(types.ErrInvalidProviderURI, "validating %q for valid host", val)
	}

	if u.Path != "" {
		return errors.Wrapf(types.ErrInvalidProviderURI, "path in %q should be empty", val)
	}
